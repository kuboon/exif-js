import {
  ...
  getIPTCinJPEG,
  getXMPinJPEG,
} from "https://esm.sh/jsr/@kuboon/exif@0.1.1?dev";
import { getArrayBufferFrom } from "https://esm.sh/jsr/@kuboon/exif@0.1.1/frontend?dev";
...
function getExif() {
  {
    ...
    img.addEventListener("click", async () => {
      const buf = await getArrayBufferFrom(img);
  ...
  }
  {
    ...
    img.addEventListener("click", async () => {
      const buf = await getArrayBufferFrom(img);
      const ret = getIPTCinJPEG(buf);
      if (ret === null) {
        console.error("Failed to get buffer from img2");
        return;
      }
      alert("IPTC:" + JSON.stringify(ret, null, "\t"));
    });
  }
  {
    /** @type {HTMLImageElement} */
    ...
    img.addEventListener("click", async () => {
      ...
      if (xmp === null) {
        console.error("Failed to get buffer from img3");
        return;
      }
      alert("XMP:" + xmp);
    });
  ...
  }
  {
    ...
    const buttonElem = document.getElementById("base64test");
    buttonElem.addEventListener("click", async () => {
      ...
      const buf = await getArrayBufferFrom(dataUrl);
      const ret = exif.getEXIFminimalTagsInJPEG(buf);
      const kv = exif.buildKeyValue(ret.tags);
      alert(JSON.stringify(kv, null, "\t"));
    });
  ...
  const testFilePath = "./DSCN0614_small.jpg";
  const blobPromise = fetch(testFilePath).then((x) => x.blob());
  {
    /** @type {HTMLButtonElement} */
    const buttonElem = document.getElementById("objecturltest");
    buttonElem.addEventListener("click", async () => {
      const blob = await blobPromise;
      const image = new Image();
      image.addEventListener("load", async () => {
        const buf = await getArrayBufferFrom(image);
        const ret = exif.getEXIFminimalTagsInJPEG(buf);
        const kv = exif.buildKeyValue(ret.tags);
        alert(JSON.stringify(kv, null, "\t"));
      });
  ...
  {
    /** @type {HTMLButtonElement} */
    const buttonElem = document.getElementById("blobtest");
    buttonElem.addEventListener("click", async () => {
      const blob = await blobPromise;
      const buf = await getArrayBufferFrom(blob);
      const ret = exif.getEXIFminimalTagsInJPEG(buf);
      const kv = exif.buildKeyValue(ret.tags);
      alert(JSON.stringify(kv, null, "\t"));
    });
