import { exif, getIPTCinJPEG, getXMPinJPEG } from "https://esm.sh/jsr/@kuboon/exif@0.1.0?dev";
import { getArrayBufferFrom } from "https://esm.sh/jsr/@kuboon/exif@0.1.0/frontend?dev";
...
function getExif() {
  {
    ...
    img.addEventListener("click", async () => {
      ...
      alert("I was taken by a" + kv["Make"].data + " " +
        kv["Model"].data);
    });
  }
  {
  ...
  }
  {
    ...
    buttonElem.addEventListener("click", async () => {
      const blob = await blobPromise;
      const buf = await getBufFromBlob(blob);
      const ret = exif.getEXIFminimalTagsInJPEG(buf);
      const kv = exif.buildKeyValue(ret.tags);
      alert(JSON.stringify(kv, null, "\t"));
  })
