...
export type PrettifyedData =
  | string
  | number
  | NumberWithRational
  | (number | NumberWithRational)[];
...
export function extractFlashBits(
  exifTags: { tagName: string; data: RawData }[],
) {
  ...
  return rows.map((row) => {
    ...
    for (const key in StringValues) {
      if (tagName === key) {
        ...
        return {
          tagName,
          data: valDict[data[0] as number] || `UnknownValue_${data[0]}`,
        };
      }
    }
    return { tagName, data };
...
export function flattenOneElemArray(
  data: string | (number | NumberWithRational)[],
): PrettifyedData {
...
export function prettifyAllData(
  rows_: RawTagEntry[],
  dict: NumDict,
): PrettifyedTagEntry[] {
