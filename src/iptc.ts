...
export function getIPTCinJPEG(file: ArrayBufferLike) {
  ...
  const iptc = scanPartialDataView(jpeg.v, new Uint8Array([0x38, 0x42, 0x49, 0x4D, 0x04, 0x04]));
  ...
  while (segmentStartPos < dataView.byteLength) {
    if (dataView.getUint8(segmentStartPos) === 0x1C && dataView.getUint8(segmentStartPos + 1) === 0x02) {
      ...
      if (segmentType in IptcFieldMap) {
        ...
        const fieldValue = getPartialString(dataView, { offset: segmentStartPos + 5, length: dataSize })!;
